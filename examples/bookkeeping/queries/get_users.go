...
func (q *Queries) GetUsers(ctx context.Context, id *int64, role *models.UserRole, status *models.AccountStatus) ([]models.Users, error) {
	rows, err := q.db.Query(ctx, get_usersSQL, id, role, status)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
